...
**실시간 혼잡 예측 + 대안 이동(MaaS) 유도 + 보행 안전/혼잡 관리**까지 확장 가능한 “스마트 모빌리티 & AI IoT 플랫폼” 컨셉의 솔루션을 제안했습니다. 
...
## 5) My Role (Tech Lead & Engineering)
3인 팀 프로젝트에서 **기술적 구현 가능성을 고려하여 기획 방향을 제시**하고, **1-day 개발 전체를 주도**했습니다.

**1. 기획 구체화 (Planning & Pivot)**
- 팀 논의 과정에서 나온 포괄적인 '여행' 주제를, 데이터 기반 해결이 가능한 **'관광지 교통 문제 해결'로 구체화(Pivot) 제안**
- 단순 아이디어를 넘어 '문제 정의-데이터 분석-솔루션'으로 이어지는 논리적 흐름 설계

**2. 개발 총괄 (End-to-End Dev)**
- 주어진 짧은 시간(1-day) 내에 결과물을 내기 위해, 팀원들은 발표 및 시각화에 집중하도록 하고 **저는 개발 전체(MVP 설계, 구현, 클라우드 배포)를 전담**하는 효율적 분업 주도
- 프론트엔드부터 서버 환경 구성까지 단독 수행

**3. 기술 커뮤니케이션**
- 구현된 기능의 기술적 원리를 팀원들에게 설명하여, 발표 자료에 정확히 반영될 수 있도록 조율
